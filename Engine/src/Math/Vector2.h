...
namespace CEMath {
	...
	public:
		...
		//	Returns a Vector that all values are 0.
		FORCEINLINE static Vector2 Zero() const;

		//	Returns a Vector that all values are 1.
		FORCEINLINE static Vector2 One() const;
	...
	template<typename TValue>
	Vector2 Vector2<TValue>::Zero() const
	{
		return Vector2(0, 0);
	}

	template<typename TValue>
	Vector2 Vector2<TValue>::One() const
	{
		return Vector2(1, 1);
	}
